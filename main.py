...
from Visualization import (
    ...
    contract_vs_churn,
    telecom_partner_vs_churn,
    PaymentMethod_vs_churn)
...
from Feature_Selection_Techniques import Feature_Selection_Techniques
...
class CustomerChurn:
    ...
    def variable_transform(self):
        try:
            ...
            # 1️⃣ Separate numerical & categorical features
            ...
            # 2️⃣ Available transformation techniques
            techniques = {
            ...
            # 3️⃣ Loop through each numerical column
            for col in X_train_num.columns:
                logger.info(f"Evaluating transformations for column: {col}")

                skewness_scores = {}

                # 4️⃣ Try each transformation
            ...
            # 7️⃣ Combine numerical + categorical
    ...
    def outliers(self):
        try:
            ...
            # 1️⃣ Separate numerical & categorical columns
            ...
            # 2️⃣ Available outlier handling techniques
            techniques = {
            ...
            # 3️⃣ Function to count outliers using IQR
            ...
            # 4️⃣ Loop through each numerical column
            for col in X_train_num.columns:
                ...
                for name, func in techniques.items():
                    try:
                        ...
                        logger.info(
                            f"Technique: {name} | Outliers after: {outlier_count}"
                        )
                    ...
                    if not outlier_scores:
                        logger.warning(
                            f"No outlier technique worked for {col}. Keeping original data."
                        )
                        continue

                # 5️⃣ Select best technique (minimum outliers)
                best_tech = min(outlier_scores, key=outlier_scores.get)
               # best_technique_per_feature[col] = best_tech

                # 6️⃣ Apply best technique
                ...
                logger.info(
                    f"Best outlier technique for {col}: {best_tech} \n "
                    f"(Outliers left: {outlier_scores[best_tech]})"
                )



            # 7️⃣ Combine numerical + categorical columns back
            self.X_train = pd.concat([X_train_final, X_train_cat], axis=1)
            self.X_test = pd.concat([X_test_final, X_test_cat], axis=1)
    ...
    def cat_to_num(self):
        try:
            ...
            # 4️⃣ Loop through each categorical column
            for col in cat_cols:
                scores = {}
                ...
                # 6️⃣ Select best encoding
                best = min(scores, key=scores.get)
                logger.info(f'Best encoding for {col}: {best}')

                # 7️⃣ Apply best encoding
        ...
        try:
                ...
                for name, func in techniques.items():
                    try:
                        ...
                        logger.info(
                            f"{name} selected {X_tr.shape[1]} features")
                ...
                if best_tech == 'variance':
                    self.X_train, self.X_test = Feature_Selection_Techniques.variance_threshold(
                        X_train_fs, X_test_fs
                    )
                elif best_tech == 'correlation':
                    self.X_train, self.X_test = Feature_Selection_Techniques.correlation_filter(
                        X_train_fs, X_test_fs, y
                    )
                else:
                    self.X_train, self.X_test = techniques[best_tech](
                        X_train_fs, X_test_fs, y
                    )
    ...
    def data_balance(self):
        try:
            ...
            logger.info(
                f'After Balancing ({best}) - Churn Distribution:\n'
                f'{self.y_train_bal.value_counts()}'
            )

            logger.info(
                f'Balanced X_train shape: {self.X_train_bal.shape} | '
                f'Balanced y_train shape: {self.y_train_bal.shape}'
            )
